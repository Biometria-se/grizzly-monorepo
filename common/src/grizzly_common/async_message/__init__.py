...
def configure_logging() -> None:
    ...
    try:
        log_file = (
            _get_log_dir()
            / f"async-messaged.{hostname()}.{datetime.now().strftime('%Y%m%dT%H%M%S%f')}.log"
        )
...
AsyncMessageRequestHandler = Callable[
    [AsyncMessageHandler, AsyncMessageRequest], AsyncMessageResponse
]
InferredAsyncMessageRequestHandler = Callable[
    [Any, AsyncMessageRequest], AsyncMessageResponse
]
...
def register(
    handlers: dict[str, AsyncMessageRequestHandler], action: str, *actions: str
) -> Callable[[InferredAsyncMessageRequestHandler], InferredAsyncMessageRequestHandler]:
    def decorator(
